...
describe('e2e: /api/health/live (env-only)', () => {
    ...
    // If a future change adds a config check to /live, this test is the thing that
    // objects: those checks belong in /api/health/ready.
...
describe('e2e: /api/health/ready (env-only)', () => {
    it.skipIf(!serverUp)('checks the env and says so', async () => {
