        ...
        return [
            {
                ...
                body: [
                    ...
                    'trip-wire, not the real safety net: nothing in this Dockerfile or a typical deploy manifest sets',
                    ...
                    '',
                    'Nitro does not await async plugins, so the server listens before migrating finishes.',
                    'The migrator therefore runs as a boot task (`server/utils/boot-tasks.ts`), and',
                    '`/api/health/ready` answers 503 until it settles, or for good if it failed.',
                    '',
                    '### Running several replicas',
                    '',
                    'The app is built to run as N identical replicas behind a load balancer with no sticky',
                    'sessions, scaled up or down at any time. What that relies on:',
                    '',
                    '- **Probes.** Readiness on `/api/health/ready`, liveness and startup on `/api/health/live`.',
                    '  Give the startup probe room for the longest migration you expect.',
                    '- **Shutdown.** On SIGTERM Nitro stops accepting connections, drains in-flight requests',
                    '  for up to `NITRO_SHUTDOWN_TIMEOUT` (default 30s), then runs the `close` hooks. The load',
                    '  balancer keeps routing to a terminating pod for a few seconds, so on Kubernetes add a',
                    '  `preStop` sleep of about 5s and set `terminationGracePeriodSeconds` above the preStop',
                    '  plus the drain timeout (e.g. 45). Never add a `process.on(\'SIGTERM\')` handler that',
                    '  exits: it cuts the drain short.',
                    '- **Old and new pods overlap during a rollout.** Old code runs against the new schema until',
                    '  the rollout finishes, so migrations must be backward compatible: add columns nullable or',
                    '  with a default, and drop or rename only in a later release once no pod reads the old',
                    '  shape (expand, then contract).',
                    '- **Connections.** Each replica holds its query pool (`NUXT_DATABASE_POOL_MAX`, default 10)',
                    '  plus one dedicated `LISTEN` connection for the cache bus, and features with their own pool',
                    '  (Mastra storage, the RAG vector store) add theirs. Budget `max_connections` for the',
                    '  replica maximum plus rollout surge.',
                    '- **Poolers.** `LISTEN` needs a session-mode connection. Behind PgBouncer in transaction',
                    '  mode, cross-replica invalidation silently degrades to the cache TTL; point',
                    '  `NUXT_DATABASE_URL` at session mode or directly at Postgres.',
                    '- **Secrets.** `NUXT_SESSION_PASSWORD` and any encryption key must be the same value on',
                    '  every replica, delivered from one shared secret.',
                    '',
